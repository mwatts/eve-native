...
impl HashIndexLeaf {
    pub fn insert(&mut self, neue_value: Interned) -> bool {
        match self {
            ...
            &mut HashIndexLeaf::Many(ref mut prev) => prev.insert(neue_value, ()).is_none(),
        }
    }
        ...
        if e > 0 {
            if let Some(hash_iter) = self.find_values(e) {
                ...
                if iter.is_better(estimate) {
                    ...
                    iter.iter =
                        OutputingIter::Single(2, OutputingIter::make_ptr(Box::new(hash_iter)));
                    true
                } else {
                    false
                }
            ...
            if let Some(hash_iter) = self.find_entities(v) {
                ...
                if iter.is_better(estimate) {
                    iter.estimate = estimate;
                    iter.iter =
                        OutputingIter::Single(0, OutputingIter::make_ptr(Box::new(hash_iter)));
                    true
                } else {
                    false
                }
        ...
        if a == 0 {
            ...
            if iter.is_better(estimate) {
                ...
                iter.iter = OutputingIter::Single(
                    1,
                    OutputingIter::make_ptr(Box::new(attrs_iter.cloned())),
                );
                true
            } else {
                false
...
impl DistinctIndex {
    ...
    pub fn is_available(&self, e: Interned, a: Interned, v: Interned) -> bool {
        ...
        match self.eavs.get(&(e, a, v)) {
            Some(rounds) => rounds.rounds.iter().fold(0, |prev, x| prev + x) > 0,
            None => false,
        }
    }
    ...
    pub fn raw_insert(
        ...
        let key = (e, a, v);
        let info = self.eavs.entry(key).or_insert_with(|| RoundEntry {
            inserted: false,
            rounds: vec![],
            active_rounds: vec![],
        });
    ...
    pub fn distinct(&mut self, input: &Change, rounds: &mut RoundHolder) {
        ...
        let needs_remove = {
            let entry = self.eavs.entry(key).or_insert_with(|| RoundEntry {
                inserted: false,
                rounds: vec![],
                active_rounds: vec![],
            });
...
fn intermediate_distinct(
    index: &mut HashMap<Vec<Interned>, IntermediateLevel, MyHasher>,
    ...
    let insert = |round, delta| match rounds.entry(round) {
        Entry::Occupied(mut ent) => {
            ...
            let val = cur
                .entry(cloned.clone())
                .or_insert_with(|| IntermediateChange {
                    key: cloned.clone(),
                    round,
                    count: 0,
                    negate,
    ...
    pub fn check(&self, key: &Vec<Interned>, value: &Vec<Interned>) -> bool {
        match self.index.get(key) {
            Some(level) => match level {
                ...
                &IntermediateLevel::Value(ref lookup) => match lookup.get(value) {
        ...
        let mut changes = vec![];
        {
            ...
            match cur {
                        ...
                        btree_map::Entry::Vacant(ent) => {
                            match cur_aggregate {
                                AggregateEntry::Empty => {
                                    ...
                                    changes.push(make_aggregate_change(
                                        &out,
                                        cur_aggregate.get_result(interner),
                                        0,
                                        round,
                                        1,
                                    ));
                ...
                &mut IntermediateLevel::SortAggregate(ref mut rounds, ref mut entry) => {
                    ...
                    if let &mut AggregateEntry::Sorted {
                        current_params: Some(ref value),
                        ref mut items,
                        changes: ref mut entry_changes,
                        ..
                    } = entry
                    {
                        ...
                        changes.extend(entry_changes.drain(..).map(|(mut value, round, count)| {
                            let result = value
                                .drain(..)
                                .map(|x| interner.internable_to_id(x))
                                .collect();
            ...
            Some(&IntermediateLevel::Value(ref lookup)) => {
                ...
                if iter.is_better(estimate) {
                    ...
                    iter.iter = OutputingIter::Multi(
                        outputs,
                        OutputingIter::make_multi_ptr(Box::new(
                            lookup.keys().cloned().collect::<Vec<_>>().into_iter(),
                        )),
                    );
                    true
                } else {
...
pub fn print_debug_table(debug_vec: &Vec<DebugEntry>) {
    ...
    columns.extend(
        cause_keys
            .iter()
            .map(|ref cause| format!(" {} ", Internable::to_string(cause))),
    );
    columns.extend(keys.iter().map(|&(ref from, ref to)| {
        format!(
            " {}->{} ",
            Internable::to_string(from),
            Internable::to_string(to)
        )
    }));
    ...
    for debug_changes in debug_vec.iter() {
        ...
        let cause_ix = cause_keys
            .iter()
            .position(|&k| k == &debug_changes.input)
            .expect(&format!(
                "Unable to find '{:?}' in causes",
                debug_changes.input
            ));
        row[cause_ix] = format!(
            " {:>width$} ",
            debug_changes.count,
            width = columns[cause_ix].len() - 2
        );
        ...
        for (key, values) in row_map.drain() {
            let ix = cause_offset
                + keys
                    .iter()
                    .position(|k| k == &(&key.0, &key.1))
                    .expect(&format!(
                        "Unable to find '{:?} -> {:?}' in keys!",
                        key.0, key.1
                    ));
            row[ix] = format!(
                " {:>width$} ",
                values.join(", "),
                width = columns[ix].len() - 2
            );
        }
        table.push(row);
    }
    ...
    println!(
        ...
        columns
            .iter()
            .map(|ref column| repeat("-").take(column.len()).collect::<String>())
            .collect::<Vec<String>>()
            .join("+")
    );
...
fn update_watch_count(
    index: &mut HashMap<Vec<Interned>, Count, MyHasher>,
    key: Vec<Interned>,
    count: Count,
) -> (Count, Count) {
