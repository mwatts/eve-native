...
lazy_static! {
    static ref FUNCTION_INFO: HashMap<String, FunctionInfo> = {
        ...
        m.insert(
            "math/absolute".to_string(),
            FunctionInfo::new(vec!["value"]),
        );
        m.insert(
            "math/mod".to_string(),
            FunctionInfo::new(vec!["value", "by"]),
        );
        m.insert(
            "math/pow".to_string(),
            FunctionInfo::new(vec!["value", "exponent"]),
        );
        m.insert(
            "math/to-fixed".to_string(),
            FunctionInfo::new(vec!["value", "to"]),
        );
        ...
        m.insert(
            "math/range".to_string(),
            FunctionInfo::multi(vec!["from", "to"], vec!["value"]),
        );
        m.insert("random/number".to_string(), FunctionInfo::new(vec!["seed"]));
        m.insert(
            "string/replace".to_string(),
            FunctionInfo::new(vec!["text", "replace", "with"]),
        );
        m.insert(
            "string/contains".to_string(),
            FunctionInfo::new(vec!["text", "substring"]),
        );
        m.insert(
            "string/lowercase".to_string(),
            FunctionInfo::new(vec!["text"]),
        );
        m.insert(
        ...
        m.insert(
            ...
            FunctionInfo::new(vec!["text", "from", "to"]),
        );
        m.insert(
            "string/split".to_string(),
            FunctionInfo::multi(vec!["text", "by"], vec!["token", "index"]),
        );
        m.insert(
            "eve-internal/string/split-reverse".to_string(),
            FunctionInfo::multi(vec!["text", "by"], vec!["token", "index"]),
        );
        m.insert(
            "string/index-of".to_string(),
            FunctionInfo::multi(vec!["text", "substring"], vec!["index"]),
        );
        m.insert("eve/type-of".to_string(), FunctionInfo::new(vec!["value"]));
        m.insert(
            "eve/parse-value".to_string(),
            FunctionInfo::new(vec!["value"]),
        );
        m.insert(
            "gather/sum".to_string(),
            FunctionInfo::aggregate(vec!["value"], vec!["sum"], FunctionKind::Sum),
        );
        m.insert(
            "gather/average".to_string(),
            FunctionInfo::aggregate(vec!["value"], vec!["average"], FunctionKind::Sum),
        );
        m.insert(
            "gather/string-join".to_string(),
            FunctionInfo::aggregate(
                vec!["value", "separator"],
                vec!["string"],
        ...
        m.insert(
            "gather/count".to_string(),
            FunctionInfo::aggregate(vec![], vec!["count"], FunctionKind::Sum),
        );
        m.insert(
            "gather/top".to_string(),
            FunctionInfo::aggregate(vec!["limit"], vec!["top"], FunctionKind::Sort),
        );
        m.insert(
            "gather/bottom".to_string(),
            FunctionInfo::aggregate(vec!["limit"], vec!["bottom"], FunctionKind::Sort),
        );
        m.insert(
            "gather/next".to_string(),
            FunctionInfo::aggregate(vec![], vec!["*"], FunctionKind::NeedleSort),
        );
        m.insert(
            "gather/previous".to_string(),
            FunctionInfo::aggregate(vec![], vec!["*"], FunctionKind::NeedleSort),
        );
        m
    };
...
impl<'a> Node<'a> {
    ...
    pub fn unify(&self, comp: &mut Compilation) {
        {
            ...
            while changed {
                ...
                for &(l, r) in comp.equalities.iter() {
                    ...
                    let left_value: Field = if let Field::Register(_) = l {
                        values.entry(l).or_insert(l).clone()
                    } else {
                        l
                    };
                    let right_value: Field = if let Field::Register(_) = r {
                    ...
                    match (left_value, right_value) {
                        ...
                        (a, b) => {
                            if a != b {
                                panic!("Invalid equality {:?} != {:?}", a, b);
                            }
                        }
                    }
                }
            }
            comp.required_fields = comp
                .required_fields
                .iter()
                .map(|v| unified_registers.get(v).unwrap().clone())
                .collect();
        }
            ...
            for (k, v) in comp.vars.iter() {
                match sub_comp.vars.entry(k.to_string()) {
                    Entry::Occupied(o) => {
                        ...
                        sub_comp
                            .equalities
                            .push((Field::Register(*v), Field::Register(*reg)));
                    }
                    Entry::Vacant(o) => {
                        o.insert(*v);
                    }
        ...
        match self {
            ...
            &mut Node::If {
                ...
                *sub_block_id = cur_block.sub_blocks.len();
                cur_block
                    .sub_blocks
                    .push(SubBlock::If(sub_block, vec![], exclusive));
                None
            }
        ...
        match self {
            ...
            &Node::AttributeAccess(ref items) => {
                ...
                for item in items[1..].iter() {
                    ...
                    cur_block
                        .constraints
                        .push(make_scan(parent, interner.string(item), next));
                    parent = next;
                }
                Some(parent)
            }
            ...
            &Node::Inequality {
                ...
                match (left_value, right_value) {
                    ...
                    _ => panic!(
                        "inequality without both a left and right: {:?} {} {:?}",
                        left, op, right
                    ),
                }
                right_value
            ...
            &Node::EmbeddedString(ref var, ref vs) => {
                ...
                if let &Some(ref name) = var {
                    ...
                    if let Field::Register(_) = out_value {
                        ...
                        cur_block
                            .constraints
                            .push(make_filter("=", out_reg, out_value));
                    }
                    cur_block
                        .constraints
                        .push(make_function("concat", resolved, out_reg));
                    Some(out_reg)
                } else {
            ...
            &Node::Infix {
                ...
                if let &Some(ref name) = result {
                    ...
                    cur_block.constraints.push(make_function(
                        op,
            ...
            &Node::RecordFunction {
                ...
                let mut cur_outputs =
                    vec![Field::Value(0); cmp::max(outputs.len(), info.outputs.len())];
                ...
                for param in params {
                    ...
                    match unwrapped {
                        &Node::Attribute(a) => compiled_params.push((a, cur_block.get_value(a))),
                        &Node::AttributeEquality(a, ref v) => {
                            let (local_pos, unwrapped) = v.to_pos_ref(span);
                            if let &Node::ExprSet(ref items) = unwrapped {
                                for item in items {
                                    compiled_params.push((
                                        a,
                                        item.compile(interner, cur_block, local_pos).unwrap(),
                                    ))
                                }
                            } else {
                                compiled_params
                                    .push((a, v.compile(interner, cur_block, local_pos).unwrap()))
                            }
                        }
                    ...
                    for (a, v) in compiled_params {
                        match info.get_index(a) {
                            ...
                            ParamType::Invalid => match (info.kind, a) {
                                (FunctionKind::Sum, "per")
                                | (FunctionKind::SortedSum, "per")
                                | (FunctionKind::Sort, "per")
                                | (FunctionKind::NeedleSort, "per") => group.push(v),
                                (FunctionKind::Sum, "for")
                                | (FunctionKind::SortedSum, "for")
                                | (FunctionKind::Sort, "for")
                                | (FunctionKind::NeedleSort, "for") => projection.push(v),
                                (FunctionKind::NeedleSort, "from") => needle.push(v),
                                _ => {
                                    cur_block.error(
                                        span,
                                        error::Error::UnknownFunctionParam(
                                            op.to_string(),
                                            a.to_string(),
                                        ),
                                    );
                ...
                let compiled_outputs: Vec<Option<Field>> = outputs
                    .iter()
                    .map(|output| {
                        output
                            .compile(interner, cur_block, span)
                ...
                for (out_ix, mut attr_output) in cur_outputs.iter_mut().enumerate() {
                    ...
                    match (&cur_value, maybe_output) {
                        ...
                        (&Field::Value(0), Some(Field::Value(_))) => {
                            let result_name =
                                format!("__eve_record_function_output{}", cur_block.id);
                            let out_reg = cur_block.get_register(&result_name);
                            cur_block.id += 1;
                            cur_block.constraints.push(make_filter(
                                "=",
                                out_reg,
                                maybe_output.unwrap(),
                            ));
                        ...
                        (&Field::Value(x), None) => {
                            ...
                            if x > 0 {
                                cur_block
                                    .constraints
                                    .push(make_filter("=", *attr_output, out_reg));
                            }
                            *attr_output = out_reg;
                        }
                        (&Field::Value(x), Some(Field::Value(z))) => {
                            if x != z {
                                panic!(
                                    "Invalid constant equality in record function: {:?} != {:?}",
                                    x, z
                                )
                            }
                            let result_name =
                ...
                match info.kind {
                    FunctionKind::Multi => {
                        cur_block.constraints.push(make_multi_function(
                            op,
                            cur_params,
                            cur_outputs,
                        ));
                    ...
                    FunctionKind::Sort | FunctionKind::Sum | FunctionKind::SortedSum => {
                        let mut sub_block = Compilation::new_child(cur_block);
                        let unified_output: Vec<Field> = cur_outputs
                            .iter()
                            .map(|x| cur_block.get_unified(x))
                            .collect();
                        sub_block.constraints.push(make_aggregate(
                            op,
                            group.clone(),
                            projection.clone(),
                            cur_params.clone(),
                            unified_output.clone(),
                            info.kind,
                        ));
                        cur_block.sub_blocks.push(SubBlock::Aggregate(
                            sub_block,
                            group,
                            projection,
                            cur_params,
                    ...
                    FunctionKind::Scalar => {
                        cur_block
                            .constraints
                            .push(make_function(op, cur_params, cur_outputs[0]));
                    }
                }
                final_result
            }
            &Node::Lookup(ref attrs, output_type) => {
                ...
                for attr in attrs {
                    ...
                    match unwrapped {
                        &Node::Attribute("entity") => {
                            entity = Some(get_provided!(cur_block, local_span, "entity"));
                        }
                        &Node::AttributeEquality("entity", ref v) => {
                            entity = v.compile(interner, cur_block, local_span);
                        }
                        _ => {}
                    }
                }
                ...
                let constraint = match (output_type, _type) {
                    (OutputType::Lookup, _) => {
                        make_scan(entity.unwrap(), attribute.unwrap(), value.unwrap())
                    }
                    (OutputType::Bind, _) => Constraint::Insert {
                        e: entity.unwrap(),
                        a: attribute.unwrap(),
                        v: value.unwrap(),
                        commit: false,
                    },
                    (OutputType::Commit, None) => Constraint::Insert {
                        e: entity.unwrap(),
                        a: attribute.unwrap(),
                        v: value.unwrap(),
                        commit: true,
                    },
                    (OutputType::Commit, Some(Field::Value(v))) => match interner.get_value(v) {
                        &Internable::String(ref s) if s == "add" => Constraint::Insert {
                            e: entity.unwrap(),
                            a: attribute.unwrap(),
                            v: value.unwrap(),
                            commit: true,
                        },
                        &Internable::String(ref s) if s == "remove" => Constraint::Remove {
                            e: entity.unwrap(),
                            a: attribute.unwrap(),
                            v: value.unwrap(),
                        },
                    ...
                    (OutputType::Commit, Some(Field::Register(_))) => Constraint::DynamicCommit {
                        e: entity.unwrap(),
                        a: attribute.unwrap(),
                        v: value.unwrap(),
                        _type: _type.unwrap(),
            ...
            &Node::LookupCommit(ref attrs) => {
                ...
                cur_block.constraints.push(make_commit_lookup(
                    entity.unwrap(),
                    attribute.unwrap(),
                    value.unwrap(),
                ));
            ...
            &Node::LookupRemote(ref attrs, output_type) => {
                ...
                let constraint = match output_type {
                    OutputType::Lookup => make_remote_lookup(
                        entity.unwrap(),
                        attribute.unwrap(),
                        value.unwrap(),
                        _for.unwrap(),
                    ...
                    OutputType::Commit => Constraint::Watch {
                    ...
                    OutputType::Bind => Constraint::Watch {
                        name: "eve/remote".to_string(),
                        registers: vec![
                            to.unwrap(),
                            _for.unwrap(),
            ...
            &Node::Record(ref var, ref attrs) => {
                ...
                for attr in attrs {
                    ...
                    let (a, v) = match unwrapped {
                        ...
                        &Node::Attribute(a) => {
                            (interner.string(a), get_provided!(cur_block, local_span, a))
                        }
                        &Node::AttributeEquality(a, ref v) => {
                            ...
                            let result = match unwrapped {
                                &Node::RecordSet(ref records) => {
                                    for record in records[1..].iter() {
                                        let cur_v = record
                                            .compile(interner, cur_block, local_span)
                                            .unwrap();
                                ...
                                &Node::ExprSet(ref items) => {
                                    for value in items[1..].iter() {
                                        let cur_v =
                                            value.compile(interner, cur_block, local_span).unwrap();
                                        cur_block.constraints.push(make_scan(reg, result_a, cur_v));
                                    }
            ...
            &Node::OutputRecord(ref var, ref attrs, ref output_type) => {
                                ...
                                &Node::RecordSet(ref records) => {
                                    ...
                                    for (ix, record) in records[1..].iter().enumerate() {
                                        let cur_v = record
                                            .compile(interner, cur_block, local_span)
                                            .unwrap();
                                        cur_block.constraints.push(Constraint::Insert {
                                            e: cur_v,
                                            a: auto_index,
                                            v: interner.number((ix + 2) as f32),
                                            commit,
                                        });
                                        cur_block.constraints.push(Constraint::Insert {
                                            e: reg,
                ...
                if needs_id {
                    cur_block
                        .constraints
                        .push(make_function("gen_id", identity_attrs, reg));
                }
                Some(reg)
            }
            &Node::RecordUpdate {
                ...
                let (reg, attr) = match unwrapped {
                    ...
                    &Node::Variable(v) => (get_provided!(cur_block, local_span, v), None),
                ...
                match (attr, val) {
                    ...
                    (Some(attr), &Node::Tag(t)) => {
                        ...
                        cur_block
                            .constraints
                            .push(make_scan(reg, interner.string(attr), me));
                        cur_block.constraints.push(Constraint::Insert {
                            e: me,
                    ...
                    (Some(attr), &Node::NoneValue) => {
                        avs.push((interner.string(attr), Field::Value(0)))
                    }
                    (Some(attr), &Node::ExprSet(ref nodes)) => {
                        for node in nodes {
                            avs.push((
                                interner.string(attr),
                                node.compile(interner, cur_block, local_span).unwrap(),
                            ))
                        }
                    }
                    ...
                    (Some(attr), &Node::OutputRecord(Some(ref name), ..)) => match op {
                        &"<-" => {
                        ...
                        _ => {
                            avs.push((
                                interner.string(attr),
                                val.compile(interner, cur_block, local_span).unwrap(),
                            ));
                        }
                    },
                    (Some(attr), v) => avs.push((
                        interner.string(attr),
                        v.compile(interner, cur_block, local_span).unwrap(),
                    )),
                    // @TODO: this doesn't handle the case where you do
                    // foo.bar <- [#zomg a]
                    (None, &Node::OutputRecord(..)) => match op {
                        &"<-" => {
                            val.compile(interner, cur_block, local_span);
                        }
                ...
                for (a, v) in avs {
                    match (*op, a, v) {
                        (":=", Field::Value(0), Field::Value(0)) => {
                            cur_block
                                .constraints
                                .push(Constraint::RemoveEntity { e: reg });
                        }
                        (":=", _, Field::Value(0)) => {
                            cur_block
                                .constraints
                                .push(Constraint::RemoveAttribute { e: reg, a });
                        }
                        (":=", _, _) => {
                            cur_block
                                .constraints
                        ...
                        ("-=", _, _) => {
                            cur_block
                                .constraints
                                .push(Constraint::Remove { e: reg, a, v });
                        }
            ...
            &Node::Not(sub_block_id, ref items) => {
                let sub_block =
                    if let SubBlock::Not(ref mut sub) = cur_block.sub_blocks[sub_block_id] {
                        sub
                    } else {
            ...
            &Node::IfBranch {
                ...
                if let SubBlock::IfBranch(ref mut sub_block, ref mut result_fields) =
                    cur_block.sub_blocks[sub_block_id]
                {
                    ...
                    if let &Node::ExprSet(ref nodes) = unwrapped {
                        for node in nodes {
                            result_fields
                                .push(node.compile(interner, sub_block, local_span).unwrap());
                        }
                    } else {
                        result_fields
                            .push(result.compile(interner, sub_block, local_span).unwrap());
                    }
                } else {
                    panic!("Wrong SubBlock type for Not");
                };
            ...
            &Node::If {
                        ...
                        .map(|cur| {
                            ...
                            match value {
                                ...
                                Some(reg @ Field::Register(_)) => {
                                    cur_block.provide(reg, true);
                                    let cur_value = if let Some(val @ &Field::Value(_)) =
                                        cur_block.var_values.get(&reg)
                                    {
                                        *val
                                    } else {
                                        reg
                                    ...
                                    if let Field::Value(_) = cur_value {
                                        let result_name =
                                            format!("__eve_if_output{}", cur_block.id);
                                        let out_reg = cur_block.get_register(&result_name);
                                        cur_block.id += 1;
                                        cur_block
                                            .constraints
                                            .push(make_filter("=", out_reg, cur_value));
                                        out_reg
                                    } else {
                                        reg
                                    }
                ...
                if let SubBlock::If(ref mut sub_block, ref mut out_registers, ..) =
                    cur_block.sub_blocks[sub_block_id]
                {
                ...
                for value in values {
                    ...
                    if let &Node::ExprSet(ref items) = unwrapped {
                        let registers = items
                            ...
                            .collect();
                        cur_block.constraints.push(Constraint::Watch {
                            name: name.to_string(),
                            registers,
                        });
                    }
            ...
            _ => panic!(
                "Trying to compile something we don't know how to compile {:?}",
                self
            ),
        }
    }
    ...
    pub fn sub_blocks(&self, interner: &mut Interner, parent: &mut Compilation) {
        ...
        for (ix, sub_block) in parent.sub_blocks.iter_mut().enumerate() {
            ...
            ancestor_constraints.push(self.sub_block_output(
                interner,
                sub_block,
                ix,
                &block_to_inputs[ix],
    ...
    pub fn sub_block_output(
        ...
        match block {
            ...
            &mut SubBlock::Aggregate(
                ...
                let block_name = cur_block.block_name.to_string();
                let result_id =
                    interner.string(&format!("{}|sub_block|aggregate_result|{}", block_name, ix));
    ...
    pub fn compile_sub_block(
        ...
        match block {
            &mut SubBlock::Not(ref mut cur_block) => {
                self.sub_blocks(interner, cur_block);
                let valid_ancestors = ancestor_constraints
                    .iter()
                    .filter(|x| *x != &output_constraint)
                    .cloned()
                    .collect();
            ...
            &mut SubBlock::Aggregate(
                ...
                let scan_id =
                    interner.string(&format!("{}|sub_block|aggregate_scan|{}", block_name, ix));
                ...
                let aggregate_id =
                    interner.string(&format!("{}|sub_block|aggregate|{}", block_name, ix));
                let result_id =
                    interner.string(&format!("{}|sub_block|aggregate_result|{}", block_name, ix));
                let mut result_key = vec![result_id];
                result_key.extend(group.iter());
                ...
                cur_block
                    .constraints
                    .push(make_intermediate_scan(scan_key, vec![]));
            }
            &mut SubBlock::AggregateScan(..) => {
                panic!("Tried directly compiling an aggregate scan")
            }
            ...
            &mut SubBlock::If(ref mut cur_block, _, exclusive) => {
                ...
                let branch_ids: Vec<Field> = (0..num_branches)
                    .map(|branch_ix| {
                        interner.string(&format!(
                            "{}|sub_block|if|{}|branch|{}",
                            block_name, ix, branch_ix
                        ))
                    })
                    .collect();
                for (branch_ix, sub) in cur_block.sub_blocks.iter_mut().enumerate() {
                    if let &mut SubBlock::IfBranch(ref mut branch_block, ref output_fields) = sub {
                        ...
                        branch_block
                            .constraints
                            .extend(related.iter().map(|v| v.clone()));
                        ...
                        if exclusive {
                            ...
                            if branch_ix + 1 < num_branches {
                                ...
                                branch_block.constraints.push(make_intermediate_insert(
                                    branch_key,
                                    vec![],
                                    true,
                                ));
...
pub fn get_input_constraints(
    needles: &HashSet<Field>,
...
pub fn get_input_constraints_transitive(
    needles: &HashSet<Field>,
    haystack: &Vec<Constraint>,
) -> Vec<Constraint> {
    let mut transitive_needles = needles.clone();
...
impl Compilation {
    ...
    pub fn new_child(parent: &Compilation) -> Compilation {
        let mut child =
            Compilation::new(format!("{}|{}", parent.block_name, parent.sub_blocks.len()));
...
pub fn compilation_to_blocks(
    mut comp: Compilation,
    interner: &mut Interner,
    path: &str,
    source: &str,
    debug: bool,
