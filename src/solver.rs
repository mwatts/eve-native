    ...
    fn clone(&self) -> Self {
        Solver {
            ...
            aggregates: self
                .aggregates
                .iter()
                .map(|&(ref a, ref b, ref c, ref d, e, f, g)| {
                    (a.clone(), b.clone(), c.clone(), d.clone(), e, f, g)
                })
                .collect(),
...
fn move_or_accept(
    ...
    accepts: &mut Vec<(usize, Interned)>,
) {
...
impl Solver {
    pub fn new(
        ...
        match active_scan {
            Some(&Constraint::Scan { e, a, v, .. }) => {
                ...
                if let Field::Register(ix) = a {
                    moves.push((1, ix));
                }
                if let Field::Register(ix) = v {
                    moves.push((2, ix));
                }
            }
        ...
        for (ix, constraint) in constraints.iter().enumerate() {
            ...
            if active_scan.map_or(false, |x| x == constraint) {
                continue;
            }
    ...
    pub fn run_intermediate(
        ...
        pool: &mut EstimateIterPool,
        frame: &mut Frame,
    ) {
        if !self.do_intermediate_move(frame) {
            return;
        }
    ...
    pub fn do_move(&self, state: &mut RuntimeState, frame: &mut Frame) -> bool {
        if self.moves.len() > 0 {
            ...
            for &(from, to) in self.moves.iter() {
                match from {
                    0 => {
                        frame.row.set_multi(to, change.e);
                    }
                    1 => {
                        frame.row.set_multi(to, change.a);
                    }
                    2 => {
                        frame.row.set_multi(to, change.v);
                    }
                    _ => unreachable!(),
                }
            }
            ...
            for accept in self.accepts.iter() {
                if !(*accept)(state, frame, usize::MAX) {
                    return false;
                }
            }
        }
    ...
    pub fn do_intermediate_move(&self, frame: &mut Frame) -> bool {
        if let Some(ref intermediate) = frame.intermediate {
            ...
            for &(from, value) in self.intermediate_accepts.iter() {
                if intermediate.key[from] != value {
                    return false;
                }
            }
        }
    ...
    pub fn do_remote_move(&self, frame: &mut Frame) -> bool {
        if let Some(ref remote) = frame.remote {
            for &(from, to) in self.moves.iter() {
                match from {
                    0 => {
                        frame.row.set_multi(to, remote.e);
                    }
                    1 => {
                        frame.row.set_multi(to, remote.a);
                    }
                    2 => {
                        frame.row.set_multi(to, remote.v);
                    }
                    3 => {
                        frame.row.set_multi(to, remote._for);
                    }
                    4 => {
                        frame.row.set_multi(to, remote._type);
                    }
                    5 => {
                        frame.row.set_multi(to, remote.from);
                    }
                    6 => {
                        frame.row.set_multi(to, remote.to);
                    }
                    _ => unreachable!(),
                }
            }
            for &(from, value) in self.intermediate_accepts.iter() {
                match from {
                    ...
                    3 => {
                        if remote._for != value {
                            return false;
                        }
                    }
                    4 => {
                        if remote._type != value {
                            return false;
                        }
                    }
                    5 => {
                        if remote.from != value {
                            return false;
                        }
                    }
                    6 => {
    ...
    pub fn clear_rounds(&self, state: &mut RuntimeState, frame: &mut Frame) -> bool {
        {
            ...
            if let Some(ref change) = frame.input {
                output_rounds
                    .output_rounds
                    .push((change.round, change.count));
            } else if let Some(ref change) = frame.intermediate {
                let count = if change.negate {
                    change.count * -1
                } else {
                    change.count
                };
...
pub fn make_scan_get_iterator(scan: &Constraint, ix: usize) -> Arc<GetIteratorFunc> {
    ...
    Arc::new(move |iter, state, frame| {
        ...
        if state
            .index
            .propose(iter, resolved_e, resolved_a, resolved_v)
        {
            iter.constraint = ix;
            match iter.iter {
...
pub fn make_scan_get_rounds(scan: &Constraint) -> Arc<GetRoundsFunc> {
    ...
    Arc::new(move |state, frame| {
        ...
        let iter = state
            .distinct_index
            .iter(resolved_e, resolved_a, resolved_v);
...
pub fn make_commit_lookup_get_rounds(scan: &Constraint) -> Arc<GetRoundsFunc> {
    ...
    Arc::new(move |state, frame| {
        ...
        if !state
            .distinct_index
            .is_commit(resolved_e, resolved_a, resolved_v)
        {
...
pub fn make_lookup_remote_get_iterator(scan: &Constraint, ix: usize) -> Arc<GetIteratorFunc> {
    let (e, a, v, _for, _type, from, to, register_mask) = match scan {
        &Constraint::LookupRemote {
            ...
            to,
            register_mask,
        } => (e, a, v, _for, _type, from, to, register_mask),
        _ => unreachable!(),
    };
    let mut fields = vec![];
    let mut outputs = vec![];
    if let Field::Register(ix) = e {
        fields.push(RemoteChangeField::E);
        outputs.push(ix);
    }
    if let Field::Register(ix) = a {
        fields.push(RemoteChangeField::A);
        outputs.push(ix);
    }
    if let Field::Register(ix) = v {
        fields.push(RemoteChangeField::V);
        outputs.push(ix);
    }
    if let Field::Register(ix) = _for {
        fields.push(RemoteChangeField::For);
        outputs.push(ix);
    }
    if let Field::Register(ix) = _type {
        fields.push(RemoteChangeField::Type);
        outputs.push(ix);
    }
    if let Field::Register(ix) = from {
        fields.push(RemoteChangeField::From);
        outputs.push(ix);
    }
    if let Field::Register(ix) = to {
        fields.push(RemoteChangeField::To);
        outputs.push(ix);
    }
    Arc::new(move |iter, state, frame| {
        // if we have already solved all of this scan's vars, we just move on
        ...
        if iter.is_better(state.remote_index.len()) {
            ...
            let remote_iter = state
                ...
                .map(|x| x.extract(&fields))
                .collect::<Vec<_>>();
            iter.estimate = state.remote_index.len();
            iter.iter = OutputingIter::Multi(
                outputs.clone(),
                OutputingIter::make_multi_ptr(Box::new(remote_iter.into_iter())),
            );
            iter.constraint = ix;
        }
        true
...
pub fn make_filter_accept(scan: &Constraint, me: usize) -> Arc<AcceptFunc> {
    let (left, right, func, param_mask) = match scan {
        &Constraint::Filter {
            ...
            param_mask,
            ..
        } => (left.clone(), right.clone(), *func, param_mask),
        _ => unreachable!(),
    };
    Arc::new(move |state, frame, cur_constraint| {
            ...
            match result {
                Some(v) => {
                    if iter.is_better(1) {
                        ...
                        iter.constraint = ix;
                        iter.estimate = 1;
                        iter.iter = OutputingIter::Single(
                            reg,
                            OutputingIter::make_ptr(Box::new(iter::once(id))),
                        );
                    }
                    true
                }
...
pub fn make_function_accept(scan: &Constraint, me: usize) -> Arc<AcceptFunc> {
    ...
    Arc::new(move |state, frame, cur_constraint| {
        ...
        let solved = frame.row.solved_fields;
        let solving_output_with_inputs =
            check_bits(solved, param_mask) && has_any_bits(frame.row.solving_for, output_mask);
        let solving_input_with_output = check_bits(solved, param_mask | output_mask)
            && has_any_bits(frame.row.solving_for, param_mask);
                ...
                Some(mut result_values) => {
                    ...
                    if iter.is_better(estimate) {
                        let outputs = output_fields
                            ...
                            .collect();
                        let result_vec = result_values
                            .drain(..)
                            .map(|mut row| {
                                row.drain(..)
                                    .map(|field| state.interner.internable_to_id(field))
                                    .collect()
                            })
                        ...
                        iter.iter = OutputingIter::Multi(
                            outputs,
                            OutputingIter::make_multi_ptr(Box::new(result_vec.into_iter())),
                        );
                    }
                    true
                }
...
pub fn make_intermediate_get_iterator(scan: &Constraint, ix: usize) -> Arc<GetIteratorFunc> {
    let (key, value, register_mask, output_mask) = match scan {
        &Constraint::IntermediateScan {
            ...
            output_mask,
            ..
        } => (key.clone(), value.clone(), register_mask, output_mask),
        _ => unreachable!(),
    };
...
pub fn make_intermediate_get_rounds(scan: &Constraint) -> Arc<GetRoundsFunc> {
    ...
    Arc::new(move |state, frame| {
        ...
        state.output_rounds.compute_output_rounds(
            state
                .intermediates
                .distinct_iter(&resolved, &resolved_value),
        );
    })
}
...
pub fn make_anti_get_rounds(scan: &Constraint) -> Arc<GetRoundsFunc> {
    ...
    Arc::new(move |state, frame| {
        ...
        state
            .output_rounds
            .compute_anti_output_rounds(state.intermediates.distinct_iter(&resolved, &vec![]));
    })
}
...
pub fn do_commit(me: &Solver, state: &mut RuntimeState, frame: &mut Frame) {
    ...
    for &(_, count) in state.output_rounds.get_output_rounds().iter() {
        for &(e, a, v, change_type) in me.commits.iter() {
            let correct_count = if change_type == ChangeType::Remove {
                count * -1
            } else {
                count
            };
...
pub fn do_dynamic_commit(me: &Solver, state: &mut RuntimeState, frame: &mut Frame) {
    ...
    for &(_, count) in state.output_rounds.get_output_rounds().iter() {
        for &(e, a, v, _type) in me.dynamic_commits.iter() {
            let (correct_count, change_type) = if frame.resolve(&_type) == me.interned_remove {
                (count * -1, ChangeType::Remove)
            } else {
                (count, ChangeType::Insert)
            };
            let output = Change {
                e: frame.resolve(&e),
                a: frame.resolve(&a),
...
pub fn do_aggregate(me: &Solver, state: &mut RuntimeState, frame: &mut Frame) {
    for &(ref group, ref projection, ref params, ref output_key, add, remove, kind) in
        me.aggregates.iter()
    {
        ...
        let resolved_projection = if kind == FunctionKind::Sort
            || kind == FunctionKind::NeedleSort
            || kind == FunctionKind::SortedSum
        {
            projection
                .iter()
                .map(|v| state.interner.get_value(frame.resolve(v)).clone())
                .collect()
        } else {
            vec![]
        };
        let resolved_params: Vec<Internable> = {
            params
                .iter()
                .map(|v| state.interner.get_value(frame.resolve(v)).clone())
                .collect()
        ...
        for &(round, count) in state.output_rounds.get_output_rounds().iter() {
            ...
            frame.counters.inserts += 1;
            state.intermediates.aggregate(
                &mut state.interner,
                resolved_group.clone(),
                resolved_projection.clone(),
