...
impl<'a> EvePaths<'a> {
    pub fn new(
        ...
        let libraries_path = maybe_libraries.map(PathBuf::from).or_else(|| {
            if !clean {
        ...
        let programs_path = maybe_programs.map(PathBuf::from).or_else(|| {
            if !clean {
...
fn find_in_root(dir: &str) -> Option<PathBuf> {
    ...
    println!("{} Unable to find directory '{}' in ancestor and no {} path specified. Running without {}.", BrightYellow.paint("WARN:"), dir, dir, dir);
    None
}
