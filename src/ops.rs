...
impl Change {
    ...
    pub fn print(&self, interner: &Interner) -> String {
        ...
        format!(
            "Change (<{}>, {:?}, {})  {}:{}:{}",
            self.e, a, v, self.transaction, self.round, self.count
        )
    }
                ...
                | &&Constraint::LookupCommit {
                    ...
                    if actual_e != 0 {
                        ...
                        match tag_mappings.get(e) {
                            ...
                            None => scan_shapes.push(PipeShape::Scan(actual_e, actual_a, actual_v)),
                        }
                    }
                }
...
impl PartialEq for Block {
    fn eq(&self, other: &Self) -> bool {
        ...
        let my_constraints: HashSet<Constraint> =
            HashSet::from_iter(self.constraints.iter().cloned());
        let other_constraints: HashSet<Constraint> =
            HashSet::from_iter(other.constraints.iter().cloned());
...
impl fmt::Debug for Counters {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        write!(
            ...
            "     accept: {} ({})\n",
            self.accept_ns,
            (self.accept_ns as f64) / (self.total_ns as f64)
        );
        write!(f, "\n");
        write!(f, "  counts:\n");
...
impl PartialOrd for Internable {
    fn partial_cmp(&self, rhs: &Self) -> Option<cmp::Ordering> {
        ...
        match (self, rhs) {
            ...
            (&Internable::String(ref s), &Internable::String(ref s2)) => {
                Some(natord::compare(s, s2))
            }
...
impl Internable {
    ...
    pub fn print(&self) -> String {
        match self {
            &Internable::String(ref s) => s.to_string(),
            &Internable::Number(_) => Internable::to_number(self).to_string(),
            &Internable::Null => "Null!".to_string(),
        }
    }
...
impl JSONInternable {
    ...
    pub fn print(&self) -> String {
        match self {
            &JSONInternable::String(ref s) => s.to_string(),
            &JSONInternable::Number(_) => JSONInternable::to_number(self).to_string(),
            &JSONInternable::Null => "Null!".to_string(),
        }
    }
}
...
impl Interner {
    pub fn new() -> Interner {
        let mut me = Interner {
            ...
            value_to_id: vec![Internable::Null],
            next_id: 1,
        };
        me.string("tag");
        me
    ...
    pub fn number(&mut self, num: f32) -> Field {
        let bitpattern = unsafe { transmute::<f32, u32>(num) };
...
impl Constraint {
    pub fn get_registers(&self) -> Vec<Field> {
        match self {
            ...
            &Constraint::LookupRemote {
                ...
                ref to,
                ..
            } => filter_registers(&vec![e, a, v, _for, _type, from, to]),
            ...
            &Constraint::DynamicCommit {
                ...
                ref v,
                ref _type,
            } => filter_registers(&vec![e, a, v, _type]),
            &Constraint::Project { ref registers } => {
                registers.iter().map(|v| Field::Register(*v)).collect()
            }
            &Constraint::Watch { ref registers, .. } => {
                filter_registers(&registers.iter().collect())
            }
        }
    }
    ...
    pub fn get_output_registers(&self) -> Vec<Field> {
        match self {
            ...
            &Constraint::MultiFunction { ref outputs, .. } => {
                filter_registers(&outputs.iter().collect())
            }
            &Constraint::Aggregate { ref output, .. } => filter_registers(&output.iter().collect()),
            &Constraint::IntermediateScan { ref value, .. } => {
    ...
    pub fn replace_registers(&mut self, lookup: &HashMap<Field, Field>) {
        match self {
            ...
            &mut Constraint::IntermediateScan {
                ref mut full_key,
                ref mut key,
                ref mut value,
                ref mut register_mask,
            ...
            &mut Constraint::InsertIntermediate {
                ref mut key,
                ref mut value,
                ..
            } => {
            ...
            &mut Constraint::RemoveAttribute {
                ref mut e,
                ref mut a,
            } => {
                replace_registers(&mut vec![e, a], lookup);
            }
            &mut Constraint::RemoveEntity { ref mut e } => {
                replace_registers(&mut vec![e], lookup);
            }
            &mut Constraint::DynamicCommit {
                ref mut e,
                ref mut a,
                ref mut v,
                ref mut _type,
            } => {
                replace_registers(&mut vec![e, a, v, _type], lookup);
            }
            ...
            &mut Constraint::Watch {
                ref mut registers, ..
            } => {
                replace_registers(&mut registers.iter_mut().collect(), lookup);
            }
        }
    }
}
...
impl Clone for Constraint {
    fn clone(&self) -> Self {
        match self {
            ...
            &Constraint::Project { ref registers } => Constraint::Project {
...
impl PartialEq for Constraint {
    fn eq(&self, other: &Constraint) -> bool {
        match (self, other) {
            ...
            (
                &Constraint::AntiScan { ref key, .. },
                &Constraint::AntiScan { key: ref key2, .. },
            ) => key == key2,
            (
                &Constraint::IntermediateScan { ref full_key, .. },
                &Constraint::IntermediateScan {
                    full_key: ref full_key2,
                    ..
                },
            ) => full_key == full_key2,
            (
            ...
            ) => op == op2 && output == output2 && params == params2,
            (
                &Constraint::MultiFunction {
                    ref op,
                    ref outputs,
            ...
            ) => op == op2 && outputs == outputs2 && params == params2,
            (
                &Constraint::Aggregate {
                    ref op,
                    ref output,
            ...
            ) => key == key2 && value == value2 && negate == negate2,
            (
            ...
            (
                ...
                &Constraint::RemoveAttribute { e: e2, a: a2 },
            ) => e == e2 && a == a2,
            ...
            ) => e == e2 && a == a2 && v == v2 && _type == type2,
            (
                &Constraint::Project { ref registers },
                &Constraint::Project {
                    registers: ref registers2,
                },
            ...
            ) => name == name2 && registers == registers2,
            _ => false,
        }
    }
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            &Constraint::LookupRemote {
                ...
                f,
                "LookupRemote ( {:?}, {:?}, {:?}, {:?}, {:?}, {:?}, {:?} )",
                e, a, v, _for, _type, from, to
            ),
            &Constraint::AntiScan { ref key, .. } => write!(f, "AntiScan ({:?})", key),
            &Constraint::IntermediateScan {
                ref key, ref value, ..
            } => write!(f, "IntermediateScan ( {:?}, {:?} )", key, value),
            &Constraint::Insert { e, a, v, .. } => {
                write!(f, "Insert ( {:?}, {:?}, {:?} )", e, a, v)
            }
            &Constraint::InsertIntermediate {
                ref key,
                ref value,
                ...
                f,
                "InsertIntermediate ({:?}, {:?}, negate? {:?})",
                key, value, negate
            ),
            &Constraint::Remove { e, a, v, .. } => {
                write!(f, "Remove ( {:?}, {:?}, {:?} )", e, a, v)
            }
            &Constraint::RemoveAttribute { e, a, .. } => {
                write!(f, "RemoveAttribute ( {:?}, {:?} )", e, a)
            }
            &Constraint::RemoveEntity { e, .. } => write!(f, "RemoveEntity ( {:?} )", e),
            &Constraint::DynamicCommit { e, a, v, _type, .. } => {
                write!(f, "Remove ( {:?}, {:?}, {:?}, {:?} )", e, a, v, _type)
            }
            &Constraint::Function {
                ref op,
                ref params,
                ref output,
                ..
            } => write!(f, "{:?} = {}({:?})", output, op, params),
            &Constraint::MultiFunction {
                ref op,
                ref params,
                ref outputs,
                ..
            } => write!(f, "{:?} = {}({:?})", outputs, op, params),
            &Constraint::Aggregate {
                ref op,
                ref group,
                ref projection,
                ...
                f,
                "{:?} = {}(per: {:?}, for: {:?}, {:?})",
                output_key, op, group, projection, params
            ),
            &Constraint::Filter {
                ref op,
                ref left,
...
pub fn math_range(params: Vec<&Internable>) -> Option<Vec<Vec<Internable>>> {
    match params.as_slice() {
        &[&Internable::Number(_), &Internable::Number(_)] => {
            ...
            Some(
                (from..to + 1)
                    .map(|x| vec![Internable::from_number(x as f32)])
                    .collect(),
            )
        }
        _ => None,
...
pub fn string_length(params: Vec<&Internable>) -> Option<Internable> {
    match params.as_slice() {
        &[&Internable::String(ref text)] => Some(Internable::from_number(
            UnicodeSegmentation::graphemes(text.as_str(), true).count() as f32,
        )),
        _ => None,
    }
}
    ...
    match params_slice {
        &[&Internable::String(ref text), ..] => {
            let graphemes: Vec<&str> =
                UnicodeSegmentation::graphemes(text.as_str(), true).collect();
            let length = graphemes.len();

            let (from, to) = match params_slice {
                &[_, &Internable::Number(_), &Internable::Number(_)] => (
                    Internable::to_number(params[1]) as isize,
                    Internable::to_number(params[2]) as isize,
                ),
                &[_, _, &Internable::Number(_)] => {
                    (1 as isize, Internable::to_number(params[2]) as isize)
                }
                &[_, &Internable::Number(_), _] => (
                    Internable::to_number(params[1]) as isize,
                    (length + 1) as isize,
                ),
            ...
            let start = if from < 1 {
                length - from.abs() as usize
            } else {
                (from - 1) as usize
            };
            let end = if to < 1 {
                length - to.abs() as usize
            } else {
                (to - 1) as usize
            };
    ...
    match params.as_slice() {
        &[&Internable::String(ref text), &Internable::String(ref substring)] => {
            let results = text
                ...
                .map(|(ix, _)| vec![Internable::from_number((ix + 1) as f32)])
                .collect();
            Some(results)
        }
        _ => None,
    ...
    match params.as_slice() {
        &[&Internable::String(ref text), &Internable::String(ref by)] => {
            let results = text
                ...
                .map(|(ix, v)| {
                    vec![
                        Internable::String(v.to_string()),
                        Internable::from_number((ix + 1) as f32),
                    ]
                })
                .collect();
            Some(results)
        ...
        Some(&&Internable::String(ref s)) => {
            ...
            match result {
                ParseResult::Ok(Node::Pos(_, box Node::Float(f))) => {
                    Some(Internable::from_number(f))
                }
                ParseResult::Ok(Node::Pos(_, box Node::Integer(i))) => {
                    Some(Internable::from_number(i as f32))
                }
                _ => Some(Internable::String(s.to_owned())),
            }
        }
...
pub fn aggregate_sum_add(
    current: &mut AggregateEntry,
    params: &Vec<Internable>,
    _: &Vec<Internable>,
) {
    match params.as_slice() {
        &[ref param @ Internable::Number(_)] => {
            ...
            match current {
                &mut AggregateEntry::Result(ref mut res) => {
                    *res = *res + value;
                }
                _ => {
                    *current = AggregateEntry::Result(value);
                }
            }
        }
        _ => {}
    ...
    match params.as_slice() {
        &[ref param @ Internable::Number(_)] => {
            ...
            match current {
                ...
                _ => {
                    *current = AggregateEntry::Result(-1.0 * value);
                }
            }
        }
        _ => {}
...
pub fn aggregate_count_add(current: &mut AggregateEntry, _: &Vec<Internable>, _: &Vec<Internable>) {
    match current {
        ...
        _ => {
            *current = AggregateEntry::Result(1.0);
        }
    }
}

pub fn aggregate_count_remove(
    current: &mut AggregateEntry,
    _: &Vec<Internable>,
    _: &Vec<Internable>,
) {
    ...
    match params.as_slice() {
        &[ref param @ Internable::Number(_)] => {
            ...
            match current {
                ...
                _ => {
                    *current = AggregateEntry::Counted {
                        count: 1.0,
                        sum: value,
                        result: value,
                    };
...
pub fn aggregate_string_join_add(
    current: &mut AggregateEntry,
    params: &Vec<Internable>,
...
pub fn aggregate_string_join_remove(
    current: &mut AggregateEntry,
    params: &Vec<Internable>,
    projection: &Vec<Internable>,
) {
...
fn is_aggregate_in_round(&(_, v): &(&Vec<Internable>, &Vec<Count>), round: Round) -> bool {
    let sum: Count = v
        .iter()
        .filter(|cur| cur.abs() <= round as i32)
        .map(|&cur| if cur < 0 { -1 } else { 1 })
        .sum();
    sum > 0
}

pub fn aggregate_top_add(
    current: &mut AggregateEntry,
    params: &Vec<Internable>,
    _: &Vec<Internable>,
) {
    if let &mut AggregateEntry::Sorted {
        ref mut items,
        current_round,
        ref current_params,
        ref mut changes,
        ...
        if let &Some(ref limit_params) = current_params {
            ...
            if let Some(interned_limit @ &Internable::Number(_)) = limit_param {
                ...
                let mut iter = items
                    ...
                    .rev()
                    .filter(|entry| {
                        entry.0.last() == limit_param && is_aggregate_in_round(entry, current_round)
                    })
                    .skip(limit - 1);
...
pub fn aggregate_bottom_remove(
    current: &mut AggregateEntry,
    params: &Vec<Internable>,
    _: &Vec<Internable>,
) {
...
pub fn aggregate_next_add(
    ...
    } = current
    {
        if let Some(counts) = items.get(params) {
            let sum: Count = counts
                .iter()
                .filter(|cur| cur.abs() <= current_round as i32)
                .map(|&cur| if cur < 0 { -1 } else { 1 })
                .sum();
            if sum < 0 {
                return;
        ...
        if let &Some(_) = current_params {
            let prev = items
                .range::<Vec<Internable>, _>((Bound::Unbounded, Bound::Excluded(params)))
                .rev()
                .filter(|entry| is_aggregate_in_round(entry, current_round))
                .next();
            let next = items
                .range::<Vec<Internable>, _>((Bound::Excluded(params), Bound::Unbounded))
                .filter(|entry| is_aggregate_in_round(entry, current_round))
                .next();
            match prev {
                Some((v, prev_rounds)) => {
        ...
        if let &Some(_) = current_params {
            ...
            match prev {
                Some((v, prev_rounds)) => {
                    ...
                    if let Some((prev_next, prev_next_rounds)) = next {
                        let interesting = prev_next_rounds
                            .iter()
                            .find(|&c| *c == current_round as i32);
...
impl OutputRounds {
    ...
    fn fetch_neue_current(
        ...
        match (self.output_rounds.len(), self.prev_output_rounds.len()) {
            (0, _) => (
                &mut self.output_rounds,
                &mut self.prev_output_rounds,
                &mut self.temp_results,
            ),
            (_, 0) => (
                &mut self.prev_output_rounds,
                &mut self.output_rounds,
...
impl RoundHolder {
    pub fn new() -> RoundHolder {
        ...
        RoundHolder {
            rounds,
            commits: HashMap::new(),
            staged_commit_keys: vec![],
            collapsed_commits: CollapsedChanges::new(),
            max_round: 0,
        }
    }
    ...
    pub fn prepare_commits(
        &mut self,
        index: &mut HashIndex,
        distinct_index: &mut DistinctIndex,
    ) -> bool {
                ...
                Some(&(
                    ...
                    if count < 0 {
                        ...
                        match (a, v) {
                            ...
                            _ => {
                                panic!("Staged remove that is completely filled in");
                            }
                        }
                    }
        ...
        for info in self.commits.values() {
            match info {
                &(ChangeType::Insert, Change { count, .. }) => {
                    if count > 0 {
                        self.collapsed_commits.insert(info.1);
                    }
                }
    ...
    pub fn get_round(&mut self, holder: &mut RoundHolder, round: Round) -> &Vec<Change> {
        {
            ...
            cur_changes.clear();
            self.change_ix = 0;
            for (_, change) in holder.rounds[round as usize]
                .drain()
                .filter(|v| v.1.count != 0)
            {
                cur_changes.push(change);
            }
        }
        ...
        match self {
            ...
            &RunLoopMessage::RemoteTransaction(ref changes) => {
                let stringified_changes = changes
                    ...
                    .join("\n- ");
                format!(
                    "`Remote transaction` with changes: \n- {}",
                    stringified_changes
                )
            }
            &RunLoopMessage::CodeTransaction(ref added_blocks, ref removed_blocks) => {
                let added_block_names = added_blocks
                    ...
                    .join(", ");
                format!(
                    "`Code transaction` with {} blocks added: \n{}\n\tand {} blocks removed: {}",
                    added_blocks.len(),
                    added_block_names,
                    removed_blocks.len(),
                    removed_blocks.join(", ")
...
pub enum MetaMessage {
    Transaction {
        ...
        outputs: Vec<RawChange>,
    },
}
impl MetaMessage {
    pub fn collapse(self) -> MetaMessage {
        match self {
            MetaMessage::Transaction { inputs, outputs } => MetaMessage::Transaction {
...
impl Program {
    ...
    pub fn exec_query(&mut self, name: &str) -> Vec<Interned> {
        ...
        self.block_info
            .get_block(name)
            .run(&mut self.state, &mut iter_pool, &mut frame);
    ...
    pub fn register_block(&mut self, mut block: Block) {
        ...
        self.block_info
            .block_names
            .insert(block.name.to_string(), ix);
    ...
    pub fn unregister_block(&mut self, name: String) {
        if let Some(block_ix) = self.block_info.block_names.remove(&name) {
            ...
            if let Some(neue) = self.block_info.blocks.get(block_ix) {
                self.block_info
                    .block_names
                    .insert(neue.name.to_owned(), block_ix);
            }
                    ...
                    match shape {
                        &PipeShape::Scan(e, a, v) => {
                            self.block_info
                                ...
                                .get_mut(&(e, a, v))
                                .unwrap()
                                .retain(|x| x.block != block.block_id);
                        }
                        &PipeShape::Intermediate(id) => {
                            self.block_info
                                .intermediate_pipe_lookup
    ...
    pub fn attach(&mut self, watcher: Box<Watcher + Send>) {
        ...
        println!(
            "[{}] {} {}",
            &self.name,
            BrightCyan.paint("Loaded Watcher:"),
            name
        );
...
fn intermediate_flow(
    frame: &mut Frame,
    state: &mut RuntimeState,
    block_info: &BlockInfo,
    iter_pool: &mut EstimateIterPool,
    current_round: Round,
    max_round: &mut Round,
) {
    ...
    *max_round = cmp::max(*max_round, intermediate_max);
    if let Some(_) = state.intermediates.rounds.get(&current_round) {
        let mut remaining: Vec<(Vec<Interned>, IntermediateChange)> = state
            .intermediates
            .rounds
...
fn transaction_flow_meta(
    commits: &mut Vec<Change>,
    frame: &mut Frame,
    iter_pool: &mut EstimateIterPool,
    program: &mut Program,
    maybe_meta: Option<&mut MetaMessage>,
) {
    {
        ...
        while next_frame {
            ...
            while current_round <= max_round {
                ...
                for change in round.iter() {
                    ...
                    if change.count > 0 {
                        if program.state.distinct_index.insert_active(
                            change.e,
                            change.a,
                            change.v,
                            change.round,
                            ...
                            if let Some(&mut MetaMessage::Transaction {
                                ref mut outputs, ..
                            }) = maybe_meta
                            {
                                if added {
                    ...
                    if change.count < 0 {
                        if program.state.distinct_index.remove_active(
                            change.e,
                            change.a,
                            change.v,
                            change.round,
            ...
            next_frame = program
                .state
                .rounds
                .prepare_commits(&mut program.state.index, &mut program.state.distinct_index);
        }
    }
...
impl<'a> Transaction<'a> {
    ...
    pub fn exec(
        &mut self,
        program: &mut Program,
        persistence_channel: &mut Option<Sender<PersisterMessage>>,
    ) {
    ...
    pub fn exec_meta(
        ...
        if let Some(&mut MetaMessage::Transaction { ref mut inputs, .. }) = maybe_meta {
            inputs.extend(
                self.changes
                    .iter()
                    .map(|c| c.to_raw(&program.state.interner)),
            );
        }
        for change in self.changes.iter() {
            program
                .state
                .distinct_index
                .distinct(&change, &mut program.state.rounds);
        }
        transaction_flow_meta(
            &mut self.commits,
            &mut self.frame,
            self.iter_pool,
            program,
    ...
    pub fn exec(&mut self, program: &mut Program, to_add: Vec<Block>, to_remove: Vec<String>) {
        ...
        for add in to_add {
            ...
            program
                ...
                .last()
                .unwrap()
                .run(&mut program.state, iter_pool, frame);
        }
...
impl PortableField {
    pub fn intern(&self, interner: &mut Interner) -> Field {
        match self {
            ...
            &PortableField::Value(ref internable) => {
                Field::Value(interner.internable_to_id(internable.clone()))
            }
        }
    }
    pub fn to_eve_value(&self, block: &Internable, changes: &mut Vec<RawChange>) -> Internable {
        match self {
            &PortableField::Register(ix) => {
                let id = gen_id(vec![
                    &Internable::String("register".to_owned()),
                    block,
                    &Internable::from_number(ix as f32),
                ])
                .unwrap();
            ...
            &PortableField::Value(ref v) => v.clone(),
        }
    }
}
impl Field {
    ...
    pub fn intern(&self, interner: &mut Interner) -> Constraint {
        match self {
            &PortableConstraint::Scan(ref e, ref a, ref v) => {
                make_scan(e.intern(interner), a.intern(interner), v.intern(interner))
            }
            &PortableConstraint::Output(ref e, ref a, ref v, commit) => Constraint::Insert {
                e: e.intern(interner),
                a: a.intern(interner),
                v: v.intern(interner),
                commit,
            },
            &PortableConstraint::Watch(ref name, ref registers) => Constraint::Watch {
                name: name.to_owned(),
                registers: registers.iter().map(|v| v.intern(interner)).collect(),
            },
    ...
    pub fn to_portable(&self, i: &Interner) -> PortableConstraint {
        match self {
            &Constraint::Scan {
                ...
                ref v,
                ..
            } => PortableConstraint::Scan(e.to_portable(i), a.to_portable(i), v.to_portable(i)),
            &Constraint::Filter {
                ref op,
                ref left,
                ref right,
                ...
                PortableConstraint::Filter(op.to_owned(), left.to_portable(i), right.to_portable(i))
            }
            &Constraint::Insert {
                ref e,
                ref a,
            ...
            &Constraint::Remove {
                ...
                ref a,
                ref v,
            } => PortableConstraint::Remove(e.to_portable(i), a.to_portable(i), v.to_portable(i)),
            &Constraint::RemoveAttribute { ref e, ref a } => {
                PortableConstraint::RemoveAttribute(e.to_portable(i), a.to_portable(i))
            }
            &Constraint::RemoveEntity { ref e } => {
                PortableConstraint::RemoveEntity(e.to_portable(i))
            }
            &Constraint::Watch {
                ref name,
                ref registers,
            } => PortableConstraint::Watch(
                name.to_owned(),
                registers.iter().map(|v| v.to_portable(i)).collect(),
            ),
            &Constraint::Function {
                ref op,
                ref output,
                ref params,
                ..
            } => PortableConstraint::Function(
                op.to_owned(),
                output.to_portable(i),
                params.iter().map(|v| v.to_portable(i)).collect(),
            ),
            &Constraint::MultiFunction {
                ref op,
                ref outputs,
                ref params,
                ..
            } => PortableConstraint::MultiFunction(
                op.to_owned(),
                outputs.iter().map(|v| v.to_portable(i)).collect(),
                params.iter().map(|v| v.to_portable(i)).collect(),
            ),
            _ => unimplemented!(),
        }
...
impl Block {
    pub fn to_portable(&self, interner: &Interner) -> PortableBlock {
        let constraints = self
            .constraints
            .iter()
            .map(|c| c.to_portable(interner))
            .collect();
        PortableBlock {
            name: self.name.clone(),
            block_id: interner.get_value(self.block_id).clone(),
            constraints,
        }
    }
}
...
impl Persister {
    ...
    pub fn send(&self, changes: Vec<RawChange>) {
        self.outgoing
            .send(PersisterMessage::Write(changes))
            .unwrap();
    }
