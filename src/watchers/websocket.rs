...
impl Watcher for WebsocketClientWatcher {
    ...
    fn on_diff(&mut self, interner: &mut Interner, diff: WatchDiff) {
        let adds: Vec<Vec<JSONInternable>> = diff
            .adds
            .iter()
            .map(|row| row.iter().map(|v| interner.get_value(*v).into()).collect())
            .collect();
        let removes: Vec<Vec<JSONInternable>> = diff
            .removes
            .iter()
            .map(|row| row.iter().map(|v| interner.get_value(*v).into()).collect())
            .collect();
        let text = serde_json::to_string(
            &json!({"type": "diff", "adds": adds, "removes": removes, "client": self.client_name}),
        )
        .unwrap();
