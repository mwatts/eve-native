...
use super::super::ops::{
    Internable, Interner, MetaMessage, ProgramRunner, RawChange, RunLoop, RunLoopMessage,
};
use rand::{self, Rng};
...
impl EditorWatcher {
    pub fn new(
        client_runner: &mut ProgramRunner,
        ...
        editor_runner
            .program
            .attach(Box::new(CompilerWatcher::new(client_out.clone(), true)));

        editor_runner
            .program
            .attach(Box::new(SystemTimerWatcher::new(editor_out.clone())));
        editor_runner
            .program
            .attach(Box::new(RawTextCompilerWatcher::new(editor_out.clone())));
        editor_runner
            .program
            .attach(Box::new(WebsocketClientWatcher::new(
                ws_out.clone(),
                &editor_name,
            )));
        editor_runner
        ...
        editor_runner.program.attach(Box::new(RemoteWatcher::new(
            &editor_name,
            &router.lock().unwrap().deref(),
        )));
        editor_runner
            .program
            .attach(Box::new(EditorHostWatcher::new(client_out.clone())));

        let text = serde_json::to_string(
            &json!({"type": "load-bundle", "bundle": "programs/editor", "client": &editor_name}),
        )
        .unwrap();
        ...
        let transaction = RunLoopMessage::Transaction(vec![
            RawChange {
                e: editor_record.clone(),
                a: to_s("tag"),
                v: to_s("editor/program"),
                n: to_s("editor/init"),
                count: 1,
            },
            RawChange {
        ...
        let meta_thread = EditorWatcher::make_meta_thread(
            &format!("{}-meta-receiver", editor_name),
            incoming_meta,
            editor_out.clone(),
        );
    ...
    pub fn make_meta_thread(
        ...
        thread::Builder::new()
            ...
            .spawn(move || {
                ...
                let ignored_tags: HashSet<Internable> = vec!["html/instance", "editor/tag-metrics"]
                    .iter()
                    .map(|str| Internable::String(str.to_string()))
                    .collect();
                    ...
                    match incoming.recv() {
                        Ok(MetaMessage::Transaction { inputs, outputs }) => {
                            ...
                            for input in inputs.iter() {
                                ...
                                let input_id =
                                    Internable::String(format!("{}|input|{:?}", event, input.e));
                                let av_id = Internable::String(format!(
                                    "{}|input|{:?}|av|{:?}|{:?}",
                                    event, input.e, input.a, input.v
                                ));
                            ...
                            for output in outputs.iter() {
                                if ignored_entities.contains(&output.e) {
                                    continue;
                                }
                                ...
                                // @FIXME: don't use debug print here.
                                let output_id =
                                    Internable::String(format!("{}|output|{:?}", event, output.e));
                                let av_id = Internable::String(format!(
                                    "{}|output|{:?}|av|{:?}|{:?}",
                                    event, output.e, output.a, output.v
                                ));
                                changes.push(make_change(
                                    event_id.clone(),
                                    "output",
