...
impl CompilerWatcher {
    ...
    pub fn add_constraint(
        ...
        id: Interned,
        block: Interned,
        damaged_constraints: &mut HashSet<Interned>,
        damaged_blocks: &mut HashSet<Interned>,
        params: ConstraintParams,
    ) {
        let constraints = self
            .block_to_constraints
            .entry(block)
            .or_insert_with(|| vec![]);
        constraints.push(id);
    ...
    pub fn compile_constraint(
        ...
        match constraint {
            &ConstraintParams::Scan(e, a, v) => Some(make_scan(
                self.get_field(e),
                self.get_field(a),
                self.get_field(v),
            )),
            &ConstraintParams::Output(e, a, v) => Some(Constraint::Insert {
                e: self.get_field(e),
                a: self.get_field(a),
                v: self.get_field(v),
                commit,
            ...
            &ConstraintParams::Function(name, output, ref args) => {
                let op = interner
                    .get_string(name)
                    .expect("Unable to resolve name of function.");
                let arg_fields: HashMap<String, Field> = args
                    .iter()
                    .map(|(key, value)| (key.to_owned(), self.get_field(*value)))
                    .collect();
            ...
            &ConstraintParams::Variadic(name, output, ref args) => {
                let op = interner
                    .get_string(name)
                    .expect("Unable to resolve name of variadic.");
            ...
            &ConstraintParams::GenId(var, ref interned_attrs) => {
                let arg_fields = interned_attrs
                    .iter()
                    .map(|(key, value)| {
                        (
                            interner
                                .get_string(*key)
                                .expect("Unable to resolve attribute."),
...
impl Watcher for CompilerWatcher {
    ...
    fn on_diff(&mut self, interner: &mut Interner, diff: WatchDiff) {
            ...
            if let &Internable::String(ref kind) = interner.get_value(remove[0]) {
                match (kind.as_ref(), &remove[1..]) {
                    ...
                    ("identity", &[constraint, block, attribute, ..]) => {
                        if let Some(&mut ConstraintParams::GenId(_, ref mut identity)) =
                            self.constraint_to_params.get_mut(&(block, constraint))
                        {
                    ...
                    | ("remote-output", &[id, block, ..]) => {
                        self.constraints.remove(&(block, id)).expect(
                            format!(
                                "Unable to remove nonexistent constraint: '{:?}'",
                                interner.get_value(id)
                            )
                            .as_str(),
                        );
                ...
                match (kind.as_ref(), &add[1..]) {
                    ...
                    ("variadic", &[id, block, op, output]) => {
                        self.add_constraint(
                            ...
                            &mut damaged_constraints,
                            &mut damaged_blocks,
                            ConstraintParams::Variadic(op, output, vec![]),
                        );
                    }
                    ("gen-id", &[id, block, variable]) => {
                        self.add_constraint(
                            ...
                            &mut damaged_constraints,
                            &mut damaged_blocks,
                            ConstraintParams::GenId(variable, HashMap::new()),
                        );
                    }
                ...
                match (kind.as_ref(), &add[1..]) {
                    ("argument", &[constraint, block, attribute, value]) => {
                            ...
                            .unwrap()
                        {
                            ...
                            &mut ConstraintParams::Variadic(_, _, ref mut params) => {
                                let ix =
                                    Internable::to_number(interner.get_value(attribute)) as usize;
                                params[ix] = value;
                            }
                    ...
                    ("identity", &[constraint, block, attribute, value]) => {
                        if let &mut ConstraintParams::GenId(_, ref mut identity) = self
                            .constraint_to_params
                            .get_mut(&(block, constraint))
                            .unwrap()
                        {
        ...
        for block in damaged_blocks.iter() {
            ...
            if self.block_types.contains_key(block) {
                ...
                comp.constraints.extend(
                    constraints
                        .iter()
                        .map(|&id| self.constraints.get(&(*block, id)).unwrap())
                        .cloned(),
                );
                comp.finalize();
                added_blocks.extend(compilation_to_blocks(
                    comp,
                    interner,
                    "compiler_watcher",
                    "",
        ...
        if self.remote {
            let remote_adds = added_blocks
                .iter()
                .map(|b| b.to_portable(interner))
                .collect();
            self.outgoing
                .send(RunLoopMessage::RemoteCodeTransaction(
                    remote_adds,
                    removed_blocks,
                ))
                .unwrap();
            ...
            self.outgoing
                .send(RunLoopMessage::CodeTransaction(
                    added_blocks,
                    removed_blocks,
                ))
                .unwrap();
